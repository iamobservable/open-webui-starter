...
class AIModelsDiagnostics {
    ...
    async checkAvailableModels() {
        ...
        try {
            ...
            await this.page.waitForSelector('[data-testid="model-selector"], .model-selector, select', { timeout: 10000 });
    ...
    async testTextGeneration() {
        ...
        for (const prompt of CONFIG.testPrompts) {
            ...
            try {
                ...
                // Ожидание ответа
                await this.page.waitForSelector('.message, .chat-message, .response', { timeout: CONFIG.maxResponseTime });
    ...
    async testRAGIntegration() {
        ...
        try {
            ...
            const inputSelector = 'textarea[placeholder*="message"], textarea[placeholder*="сообщение"], .chat-input textarea';
            ...
            const sources = await this.page.$$eval(
                '.source, .citation, [href*="http"]',
                elements => elements.map(el => el.textContent || el.href).filter(Boolean)
            );
    ...
    printSummary() {
        ...
        console.log(`✅ Успешных тестов: ${performanceMetrics.successfulTests}/${performanceMetrics.totalTests}`);
