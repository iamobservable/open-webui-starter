...
class GPUMonitor {
            ...
            nvidia.on('close', (code) => {
                if (code === 0) {
                    ...
                    const metrics = lines.map(line => {
                        const [timestamp, name, gpuUtil, memUtil, memUsed, memTotal, temp, power] = line.split(', ');
                        return {
    ...
    async startMonitoring(intervalMs = 2000) {
        ...
        this.interval = setInterval(async () => {
            ...
            if (metrics) {
                ...
                console.log(`GPU: ${metrics.gpuUtilization}% | Память: ${metrics.memoryUsed}MB/${metrics.memoryTotal}MB (${metrics.memoryUtilization}%) | Температура: ${metrics.temperature}°C`);
            }
        }, intervalMs);
    ...
    printSummary(summary) {
        ...
        console.log(`🎮 GPU утилизация: ${summary.gpu.min}% - ${summary.gpu.max}% (среднее: ${summary.gpu.avg}%)`);
        console.log(`💾 Память: ${summary.memory.min}% - ${summary.memory.max}% (среднее: ${summary.memory.avg}%)`);
        console.log(`📈 Пиковое использование памяти: ${summary.memory.peakUsedMB} MB`);
        console.log(`🌡️  Температура: ${summary.temperature.min}°C - ${summary.temperature.max}°C (среднее: ${summary.temperature.avg}°C)`);
        console.log(`⚡ Энергопотребление: ${summary.power.min}W - ${summary.power.max}W (среднее: ${summary.power.avg}W)`);
        console.log('='.repeat(50));
    }
